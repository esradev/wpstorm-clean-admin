...
export function EmptyState({ title, description, icons = [], linkTo, className }: EmptyStateProps) {
  return (
    <div className={cn('bg-background font-sans border-border hover:border-border/80 text-center', 'border-2 border-dashed rounded-xl p-14 w-full', 'group hover:bg-muted/50 transition duration-500 hover:duration-200', className)}>
      <div className="flex justify-center isolate w-full">
        {icons.length === 3 ? (
          <>
      ...
      <h2 className="text-foreground font-medium mt-6 font-sans">{title}</h2>
      <p className="text-sm text-muted-foreground mt-1 whitespace-pre-line font-sans">{description}</p>
      {linkTo && (
        <Link to={linkTo}>
          <Button variant="default" className={cn('mt-4', 'shadow-sm active:shadow-none')}>
            {__('Add item', 'wpstorm-clean-admin')}
          </Button>
        </Link>
      )}
    </div>
