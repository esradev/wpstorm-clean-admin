...
export function AdvancedForm<TFormValues extends FieldValues>({ fields, defaultValues, schema: customSchema, successMessage = __('Settings saved', 'wpstorm-clean-admin'), successDescription = __('Your settings have been updated', 'wpstorm-clean-admin'), submitButtonText = __('Save Settings', 'wpstorm-clean-admin'), resetButtonText = __('Reset', 'wpstorm-clean-admin'), hideResetButton = false, className = 'space-y-8', renderButtons, isFetching = false, route = { title: '', url: '' }, children = null, hasAccess = true, onSubmit: customOnSubmit, localStorageKey }: FormConfig<TFormValues> & { onSubmit?: (values: TFormValues) => void; localStorageKey?: string }) {
  const [saveBtnState, setSaveBtnState] = useState<'initial' | 'loading' | 'success' | 'error'>('initial')
  ...
  const { postData } = usePostData(route.restRoute || '', {
    ...
    onError: (err, { toastId }) => {
      ...
      toast.error(__('Error saving options', 'wpstorm-clean-admin'), {
        description: __('There was an error saving your options.', 'wpstorm-clean-admin'),
        id: toastId
      })
    },
    onFinally: () => {
              ...
              <form onSubmit={form.handleSubmit(handleSubmit)} className={className}>
                <div className="grid grid-cols-12 gap-6">
                  {fields.map((field, index) => (
                    <FieldRenderer key={`${String(field.name)}-${index}`} field={field} form={form} />
                  ))}
                </div>
                ...
                {renderButtons ? (
                  ...
                  <div className="flex items-center justify-end fixed bottom-5 left-8 z-10">
                    <FormSubmitButton state={saveBtnState} onReset={() => form.reset()} submitButtonText={submitButtonText} resetButtonText={resetButtonText} />
                  </div>
                )}
              </form>
            </Form>
