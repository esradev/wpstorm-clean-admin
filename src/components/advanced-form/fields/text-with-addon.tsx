  ...
  return (
    <div className="relative space-y-2">
      <div className="relative">
        <Label htmlFor={input?.name} className="absolute -top-2 right-2 bg-background px-1 text-xs font-medium">
          {input?.label}
        </Label>
        <div className="flex rounded-md overflow-hidden">
          <Input type="text" name={input?.name} id={input?.name} value={value} required={input?.required} onChange={handleValueChange} className="rounded-l-none border-l-0" placeholder={input?.placeholder} />
          <Select value={addon} onValueChange={handleAddonChange}>
            <SelectTrigger className="w-[180px] rounded-r-none">
              <SelectValue placeholder={__('Select addon', 'wpstorm-clean-admin')} />
            </SelectTrigger>
            <SelectContent>
      ...
      {input?.desc && (
        <p className="text-sm text-muted-foreground" id={`${input?.name}-description`}>
          {input?.desc}
        </p>
      )}
    </div>
