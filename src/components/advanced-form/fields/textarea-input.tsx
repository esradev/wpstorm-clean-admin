...
export function TextareaInput({ field, formField, isDisabled }: TextareaInputProps) {
  ...
  return (
    <>
      ...
      <FormControl>
        <ShadcnTextarea id={field.name} {...formField} placeholder={field.placeholder} rows={field.rows} {...formField} disabled={isDisabled} />
      </FormControl>
      {field.variables && (
        <>
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {field.variables.map((variable) => (
                <Button key={variable.value} variant="outline" type="button" size="sm" onClick={() => onAddVariable(variable)}>
                  {variable.label}
                </Button>
              ))}
            </div>
