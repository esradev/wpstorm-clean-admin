...
export function TextareaPattern({
  ...
  useEffect(() => {
    ...
    while ((match = regex.exec(fetchedText)) !== null) {
      ...
      initialPlaceholders[placeholderIndex] = {
        value:
          fetchedPlaceholders[placeholderIndex]?.value ||
          `{${placeholderIndex}}`,
        label:
          fetchedPlaceholders[placeholderIndex]?.label ||
          `{${placeholderIndex}}`,
      };
    }
  ...
  const renderText = () => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const isSelected =
        placeholders[placeholderIndex]?.value !== `{${placeholderIndex}}`;
      const isCurrent =
        currentPlaceholderIndex === parseInt(placeholderIndex, 10);

      parts.push(
        <span
          key={placeholderIndex}
          onClick={() => handleRemovePlaceholder(placeholderIndex)}
          className={`relative inline-block px-2 py-1 mx-1 border rounded-md ${
        ...
        >
          <span className="inline-block">
            {placeholders[placeholderIndex]?.label}
          </span>
          {isSelected && (
            <Button
              variant="ghost"
