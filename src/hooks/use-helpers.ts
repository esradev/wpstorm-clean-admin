...
const useRemoveLoader = () => {
  useEffect(() => {
    ...
    const loadingScreen = document.getElementById('wpstorm-clean-admin-dashboard-loading-container')
    if (loadingScreen) {
...
const updateActiveMenuItem = () => {
  ...
  if (!hash) {
    const defaultItem = document.querySelector('.wp-submenu li a[href="admin.php?page=wpstorm_clean_admin_settings"]')
